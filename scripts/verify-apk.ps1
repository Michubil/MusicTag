...
try {
    ...
    if ($FullNativeValidation -and ($signature -join "`n") -notmatch 'Verified using v2 scheme .*: true') { throw 'V2 signature is missing.' }
    ...
    if (($badging -join "`n") -match (Get-DeniedStoragePermissionPattern)) {
    ...
    Invoke-CheckedNative -Executable (Join-Path $context.BuildTools 'zipalign.exe') -Arguments @('-c', '4', $apk)
    ...
    try {
        ...
        foreach ($entry in $archive.Entries) {
            $name = $entry.FullName
            if ($name -match '(^|/)(scripts|apk|docs|\.local-signing|\.gradle|\.kotlin|\.android-sdk)/|\.(ps1|bat|cmd|sh|keystore|jks)$|(^|/)DebugProbesKt\.bin$') {
            ...
            if ($FullNativeValidation -and $name -match '\.so$') {
