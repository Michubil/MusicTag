...
| `.\scripts\build-apk.ps1` | 上述检查 + R8 Release、验包 |
...
验包检查版本、签名、非调试 Release、权限、ARM64、普通 ZIP 对齐、许可与开发文件排除。安装包在 `app/build/outputs/apk/release/`，不复制、不计算摘要。打包入口已包含测试，无需先重复调用 test.ps1。BuildSupport.ps1 和两个边界脚本均由上述入口调用。
